...
export async function generateStaticParams() {
  return userss.map((user) => ({ id: user.Title }));
}

// Main UserPage component
const UserPage = async ({ params }) => {
  const post = userss.find((user) => user.Title === params.id); // Find the user by ID
  ...
  return (
    <div className="bg-gray-50 min-h-screen flex flex-col">
      ...
      <header className="bg-white shadow-md py-4 h-auto ">
        <h1 className="text-lg md:text-3xl  p-1 font-sans text-[#fdfbff] text-center   ">
          {post.Question}
        </h1>
      </header>
      ...
      <main className="container mx-auto px-4 py-8 flex flex-col lg:flex-row mt-10  ">
        <div className="w-[100%] px-1 flex  flex-col">
          ...
          <h1 className="mt-10 bg-[#5e77bb] p-1 font-bold rounded-md text-white">
            Questions{" "}
          </h1>

          <div className="flex flex-col justify-between h-auto  ">
            <span className="mt-10   font-semibold text-blue-800   ">
              {" "}
              1. {post.question_One}
            </span>
            <span className="mt-10   font-semibold text-blue-800   ">
            ...
            <span className="mt-10   font-semibold text-blue-800   ">
              5. {post.question_Five}
            </span>
          </div>

          <h1 className="mt-10 bg-yellow-300 p-1 font-bold rounded-md mb-10">
            Answer{" "}
          </h1>

          <div className="flex flex-col justify-between h-auto  ">
            <span className="mt-10   font-semibold text-green-600   ">
              {" "}
              1. {post.answer}
            </span>
            <span className="mt-10   font-semibold text-green-600   ">
